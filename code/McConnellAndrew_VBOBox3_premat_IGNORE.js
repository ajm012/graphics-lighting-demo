...
function VBObox3() {
	...
	//'  vec3 reflectDirection = normalize(reflect(-lightDirection, normal));\n' + // IS THIS HOW TO CALCULATE R VECTOR?? 
	'  float lDotR = max(dot(lightDirection, reflectDirection), 0.0);\n' +
	'  float e64 = pow(lDotR, float(u_MatlSet[0].shiny));\n'+
	//'  vec3 H = normalize(lightDirection + eyeDirection); \n' + 
	//'  float nDotH = max(dot(lightDirection, normal), 0.0); \n' +
	...
	//'  vec3 reflectDirection1 = normalize(reflect(-lightDirection1, normal));\n' +
	'  float lDotR1 = max(dot(lightDirection1, reflectDirection1), 0.0);\n' +
	'  float e641 = pow(lDotR1, float(u_MatlSet[0].shiny));\n'+
    ...
    // --
	// EFFECT OF CAMERA LIGHT
	'  vec3 emissive = u_MatlSet[0].emit;\n' +
	'  vec3 ambient = u_LampSet[0].ambi * u_MatlSet[0].ambi;\n' +
	'  vec3 diffuse = u_LampSet[0].diff * v_Kd * nDotL;\n' +
	'  vec3 speculr = u_LampSet[0].spec * u_MatlSet[0].spec * e64;\n' +

    // EFFECT OF POINT LIGHT
	...
	'  gl_FragColor = vec4(emissive + ambient + diffuse + speculr + ambient1 + diffuse1 + speculr1, 1.0);\n' +
	'}\n';
...
VBObox3.prototype.init = function(myGL) {
    ...
    // THESE NEED TO BE USER CONTROLLED
    this.lamp1.I_pos.elements.set( [6.0, 5.0, 5.0]);
...
VBObox3.prototype.adjust = function(myGL, canvas) {
