...
function VBObox2() {
	...
	'  vec3 H = normalize(lightDirection + eyeDirection); \n' + // FOR BLINN-PHONG H = (L+V) / |L+V|
	'  float nDotH = max(dot(H, normal), 0.0); \n' +
	'  for(int i = 0; i < 3; i++){\n' +
	'  if(i == int(v_Mat)) {\n' +
	
	'  float e64 = pow(nDotH, float(u_MatlSet[i].shiny));\n' +
	//--
	...
	'  float e641 = pow(nDotH1, float(u_MatlSet[i].shiny));\n' +
    // --
    ...
    this.a_MatLoc;									// GPU location for 'a_Colr1' attribute
    this.a_NormLoc;
